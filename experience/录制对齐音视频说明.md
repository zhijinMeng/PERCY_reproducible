...
**A/V 录制 `audio.wav` 满幅嘶嘶杂音**：多为 stamp 切片时取了**奇数 PCM 字节**导致 16-bit 错位（仅 `record_aligned` 会出现；`record_host_mic` 直写无此问题）。已在 `stamp_aligned_recorder` 强制偶数字节切片；旧 session 需重录。
...
### 长 session 底噪 / 风扇段标定（可选）

15–20 分钟录制里风扇可能中途变响。可从 `audio.wav` 自动分出安静段与高噪段并导出噪声谱：

```bash
source /workspace/percy_ws/devel/setup.bash
rosrun percy extract_noise_profile.py /workspace/percy_data/<session_id>
# 或: python3 .../extract_noise_profile.py /path/to/audio.wav --out-dir /tmp/out
```

生成：`noise_profile_report.json`、`noise_profile_quiet.json`、`noise_profile_fan.json`（若有）、`noise_*_clip.wav`。
