...
# 持久化密钥与 ARI 配置（~/Research/.env.local），避免每次手动 export
if [[ -f "$HOST_WS/.env.local" ]]; then
  set -a
  # shellcheck source=/dev/null
  source "$HOST_WS/.env.local"
  set +a
fi
...
[[ -n "${OPENAI_API_KEY:-}" ]] && ARI_ENV+=( -e "OPENAI_API_KEY=$OPENAI_API_KEY" )
